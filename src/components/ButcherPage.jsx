...
// Aggregate total KG by meat type
const kgByMeatType = {};
...
recentOrders.forEach((order) => {
  ...
  meats.forEach((meat) => {
    ...
    kgByMeatType[meat] += kg / meats.length; // Divide KG evenly among meat types
  });
});
...
const pieData = Object.entries(kgByMeatType).map(([meat, kg]) => ({
  ...
  const [editOrderId, setEditOrderId] = useState(null); // <-- NEW: to track editing mode
...
const ORDERS_PER_PAGE = 5;
  ...
  return (
    <div className="min-h-screen bg-black text-white flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-4xl p-6 bg-zinc-900 rounded-xl shadow-xl">
  ...
  <span className="text-red-400">{formattedTime}</span>
        ...
        {/* Order List */}
    {/* Submitted Orders with Pagination */}
<div className="mt-10">
